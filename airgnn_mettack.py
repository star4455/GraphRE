...
for i in range(0,7):
    with open('{}--{}--airgnn.txt'.format(dataset, attack), 'a') as f:
        f.write('{}\n'.format(pos[i]))
    ...
    def printy(list, name):
        with open('{}--{}--airgnn.txt'.format(dataset, attack), 'a') as f:  # (路径，a只写)
    ...
    for i in range(times):
        ...
        # Setup airgnn Model
        model = AirGNN(pyg_data,nfeat=features.shape[1],nhid=16, K=10,alpha=0.1, nclass=labels.max() + 1, model='AirGNN',
                          dropout=0.5,lambda_amp=0.1,device=device)
