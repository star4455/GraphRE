...
for i in range(0,7):
    with open('{}--{}--graphsage.txt'.format(dataset, attack), 'a') as f:
        f.write('{}\n'.format(pos[i]))
    ...
    def printy(list, name):
        with open('{}--{}--graphsage.txt'.format(dataset, attack), 'a') as f:  # (路径，a只写)
    ...
    for i in range(times):
        ...
        # Setup graphsage Model
        model = SAGE(nfeat=features.shape[1], nhid=16, nclass=labels.max().item() + 1)
